...
public class RdPToNuSMV {
    ...
    public static String generateNuSMVCode(Petrinet petriNet) {
        ...
        for (Place place : petriNet.places) {
            nuSMVCode.append("  ").append(place.name).append(" : 0..").append(place.tokens).append(";\n");
        }
        nuSMVCode.append("\n");
        ...
        for (Place place : petriNet.places) {
            nuSMVCode.append("  init(").append(place.name).append(") := ").append(place.tokens).append(";\n");
        }
        nuSMVCode.append("\n");
        ...
        for (Transition transition : petriNet.transitions) {
            ...
            for (Place inputPlace : transition.inputPlaces) {
                ...
                if (transition.inputPlaces.indexOf(inputPlace) < transition.inputPlaces.size() - 1) {
                    nuSMVCode.append(" & ");
                }
            }
            nuSMVCode.append(") -> (");
            for (Place outputPlace : transition.outputPlaces) {
                nuSMVCode.append("next(").append(outputPlace.name).append(") = ").append(outputPlace.name)
                        .append(" + 1");
                if (transition.outputPlaces.indexOf(outputPlace) < transition.outputPlaces.size() - 1) {
                    nuSMVCode.append(" & ");
                }
            }
            nuSMVCode.append(");\n");
